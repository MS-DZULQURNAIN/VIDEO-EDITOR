...
async def mp3(event, msg):
    Drone = event.client
    edit = await Drone.send_message(event.chat_id, "Sedang mengconvert...", reply_to=msg.id)
...
async def file(event, msg):
    ...
    try:
        UT = time.time()
        uploader = await fast_upload(f'{name}', f'{name}', UT, Drone, edit, '**MENGUPLOAD:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG, caption=f'TELAH TERCONVERT dari: @{BOT_UN}\n\n Owner : @ms_dzulqurnain👤', force_document=True)
    except Exception as e:
        print(e)
        return await edit.edit(f"Terjadi kesalahan saat mengupload!\n\nContact [SUPPORT]({SUPPORT_LINK})")
...
async def video(event, msg):
    ...
    try:
        ...
        UT = time.time()
        uploader = await fast_upload(f'{out}', f'{out}', UT, Drone, edit, '**MENGUPLOAD:**')
        await Drone.send_file(event.chat_id, uploader, thumb=JPG2, caption=f'*TELAH TERCONVERT dari: @{BOT_UN}\n\n Owner : @ms_dzulqurnain👤', attributes=attributes, force_document=False)
    except Exception as e:
        print(e)
        return await edit.edit(f"Terjadi kesalahan saat mengupload!\n\nContact [SUPPORT]({SUPPORT_LINK})")
    await edit.delete()
