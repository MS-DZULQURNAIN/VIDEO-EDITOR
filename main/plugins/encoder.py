...
async def encode(event, msg, scale=0):
    ...
    _ps = str(f"{scale}p BERHASIL DI ENCODE")
    MSDZULQURNAIN = event.client
    edit = await MSDZULQURNAIN.send_message(event.chat_id, "Sedang Encoding...", reply_to=msg.id)
    ...
    if 'x-matroska' in mime:
        try:
            uploader = await fast_upload(f'{out2}', f'{out2}', UT, MSDZULQURNAIN, edit, '**UPLOADING:**')
        ...
        try:
            ...
            await MSDZULQURNAIN.send_file(event.chat_id, uploader, caption=text, thumb=JPG3, attributes=attributes, force_document=False)
        except Exception:
            try:
                uploader = await fast_upload(f'{out2}', f'{out2}', UT, MSDZULQURNAIN, edit, '**MENGUPLOAD:**')
                await MSDZULQURNAIN.send_file(event.chat_id, uploader, caption=text, thumb=JPG, force_document=True)
